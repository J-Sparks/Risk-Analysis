...
```{r modeling 2}
library(MASS)
colSums(is.na(URM.dummy))
apr.dummy <- URM.dummy %>%  select( PROG_APR_Rate, HS_APR_Rate, OFFER_GPA, AGE,3,5,6,7,8,9,11,12,13,1 )
num.vari <- apr.dummy[1:4]
Binary.vari <- apr.dummy[11] 
##normalized data

maxs <- apply(num.vari, 2, max);maxs
mins <- apply(num.vari, 2, min);mins
#scaled
scaled.data0 <- scale(num.vari, center =  mins, scale = maxs - mins) %>%  as.data.frame()
scaled.data <- cbind(scaled.data0, Binary.vari)
str(scaled.data)
#partition
library(caTools)
split <- sample.split(scaled.data$APR, SplitRatio =0.7 )
train <- subset(scaled.data, split == TRUE)   # Training Set
test <- subset(scaled.data, split == FALSE)  # Test Set
glimpse(train)
# factors_f
...
library(neuralnet)
apr_nn <- neuralnet(f, data = train, hidden =c(3,2),  linear.output = FALSE)
plot(apr_nn) 
#predicted train
predicted.nn.apr <- compute(apr_nn, train[2:14])
head(predicted.nn.apr$net.result,10)

predictions <- sapply(predicted.nn.apr$net.result, round)
head(predictions)
#confusion matrix
table(predictions, train$APR)


#predicted test
predicted.nn.apr <- neuralnet::compute(apr_nn, test[2:14])
head(predicted.nn.apr$net.result,10)

predictions <- sapply(predicted.nn.apr$net.result, round)
head(predictions)
#confusion matrix
table(predictions, test$APR)
